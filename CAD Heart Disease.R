...
# factor_vars <- names(which(sapply(new_CAD4, class) == "factor"))
# numeric_vars <- setdiff(colnames(new_CAD4), factor_vars)
# numeric_vars <- setdiff(numeric_vars, "Cath")
# numeric_vars
# numeric_vars_mat <- as.matrix(new_CAD4[, numeric_vars, drop=FALSE])
...
CAD4_rfcv
CAD4_cvrandomforest = randomForest(Cath~., data = CAD4.train, na.action = na.exclude, ntree=500, mtry=17)
CAD4_cvrandomforest
